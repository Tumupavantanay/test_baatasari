...
function SkillBadge({ label }: { label: string }) {
  return (
    <span className="px-2 py-0.5 bg-[] text-[] text-[10px] font-medium rounded-md border border-[]">
      {label}
    </span>
  );
}
...
export default function CandidateRow({
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-[] transition-colors border-b border-[] last:border-0">
      {/* Avatar */}
      <Avatar candidate={candidate} />

      {/* Name + meta */}
      <div className="flex-shrink-0 w-28 xl:w-36">
        <p className="text-xs font-semibold text-[] leading-tight truncate">{candidate.name}</p>
        <p className="text-[10px] text-[] truncate">
          {candidate.major} • {candidate.year}
        </p>
        {s === 'review' && (
          <p className="text-[10px] text-[] mt-0.5">{candidate.appliedTime.replace('Applied ', '')}</p>
        )}
      </div>
      ...
      {s === 'selected' && candidate.selectedDate && (
        <div className="hidden sm:block flex-shrink-0 w-28">
          ...
          <p className="text-[10px] font-semibold text-[]">{candidate.selectedDate}</p>
        </div>
      )}
      ...
      {s === 'review' && candidate.reviewedBy && (
        <div className="hidden lg:block flex-shrink-0">
          ...
          <p className="text-[10px] font-semibold text-[]">{candidate.reviewedBy}</p>
        </div>
      )}
      ...
      {s === 'interview' && (
        <div className="hidden sm:flex flex-col flex-shrink-0">
          <p className="text-[10px] text-[] mb-1">Status</p>
